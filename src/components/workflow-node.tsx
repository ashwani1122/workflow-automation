  ...
  return (
    <>
      {showToolbar && (
        ...
        <button className={`w-8 h-8 border rounded flex items-center justify-center transition-[color,background-color,transform] duration-100 ease-out active:scale-[0.95] ${isActive
                ? 'bg-[#2D2D2E] hover:bg-[#3e3e3e] border-[#5b5b5b] text-white'
                : 'bg-[#3A2A2A] hover:bg-[#4a3030] border-[#6f4f4f] text-[#ffb6b6]'}`} onClick={handleToggleNodeActive} title={isActive ? "Deactivate node" : "Activate node"} aria-label={isActive ? "Deactivate node" : "Activate node"}>
